...
def compute_rigid_transform(points_cam, points_robot):
    ...
    points_robot: Nx3, 로봇 좌표계 (base 기준, meter)
    """
...
def calibrate_rigid_transform(pipeline, align, intr, num_points=6):
    """
    1. 프레임에서 점 클릭 → 카메라 3D 좌표 자동 계산
    2. 로봇 티칭 펜던트에서 좌표(x,y,z, mm 단위)를 보고 수동으로 입력
    ...
    print("  2) 로봇 TCP를 그 실제 점에 맞추고, 티칭펜던트에서 x,y,z(mm)를 보고 입력")
    ...
    try:
        while len(cam_points) < num_points:
            ...
            if clicked["u"] is not None:
                ...
                print(f"[CALIB] 카메라 3D = {p_cam} (meter)")

                # ---- 여기부터: 로봇 좌표 수동 입력 ----
                while True:
                    s = input("[CALIB] 로봇 TCP 좌표를 mm 단위로 입력하세요 (예: 100,200,300) 또는 'r'로 다시 클릭: ").strip()
                    if s.lower().startswith('r'):
                        print("[CALIB] 이 점은 취소하고, 다시 화면에서 클릭하세요.\n")
                        p_cam = None
                        break
                    try:
                        parts = s.split(',')
                        ...
                        z_mm = float(parts[2])
                        # mm → meter 변환
                        p_robot = np.array([x_mm / 1000.0,
                                            y_mm / 1000.0,
                                            z_mm / 1000.0], dtype=np.float32)
                        print(f"[CALIB] 로봇 3D = {p_robot} (meter)\n")
                        break
                    except Exception:
                        print("[CALIB] 입력 형식이 잘못되었습니다. 예: 100,200,300 또는 r")
...
if __name__ == "__main__":
    ...
    # 1~3. Rigid Transform Calibration (수동 로봇 좌표 입력)
    R, t = calibrate_rigid_transform(pipeline, align, intr, num_points=6)
